...
ifeq ("$(origin V)", "command line")  #根据传入命令中 V  确定 指令编译状态， 1.输出详细编译过程 2.输出编译指令和提示，3.只输出提示，4.什么有都不提示
  KBUILD_VERBOSE = $(V)
endif
...
ifeq ($(KBUILD_VERBOSE),1)
  ...
  quiet=quiet_
  Q = @             #指令前加 @ 表示输出结果，不输出指令本身， 不加 @ 不仅输出结果而且输出指令本身
endif
...
ifeq ("$(origin O)", "command line")  # O=？  根据指令确定是否把输出文件放在指定文件夹
  KBUILD_OUTPUT := $(O)
endif

# That's our default target when none is given on the command line  //这是我们在命令行上没有给出任何参数时的默认目标  
PHONY := _all
_all:

# Cancel implicit rules on top Makefile  //取消Makefile顶部的隐式规则
...
# Leave processing to above invocation of make  //将处理留给上面的make调用
skip-makefile := 1
...
#########################################################################
 #####    |  竖杠表示管道的意思 ， sed -e s/i.86/x86/  表示将管道输入的字符串中的“i.86”替换成"x86"
...
# Read UBOOTRELEASE from include/config/uboot.release (if it exists) //内容：2017.03-g53a52238d9
...
# To make sure we do not include .config for any of the *config targets     //为了确保我们没有为任何*config目标包含.config，尽早捕获它们， 
# catch them early, and hand them over to scripts/kconfig/Makefile           //并将它们交给scripts/kconfig/Makefile 
# It is allowed to specify more targets when calling make, including            //当调用make时，允许指定更多的目标，包括混合的*config目标和build目标
# mixing *config targets and build targets.
# For example 'make oldconfig all'.                                       // 例如'make oldconfig all'。 检测何时指定了混合目标，并进行第二次make调用，
# Detect when mixed targets is specified, and make a second invocation
# of make so .config is not included in this case either (for *config).   //在这种情况下，.config也不包括(对于*config)。
...
%config: scripts_basic outputmakefile FORCE
	$(Q)$(MAKE) $(build)=scripts/kconfig $@
# 上面两行中第一行的结果 FORCE 总是执行  make -f ./scripts/Makefile.build obj=scripts/basic
#  第二行  make -f ./scripts/Makefile.build obj=scripts/kconfig XXX_defconfig
else
# ===========================================================================
